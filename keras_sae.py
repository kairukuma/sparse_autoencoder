...
feature_images = np.zeros(enc_W.shape)

for i in range(feature_images.shape[1]):
	feature_images[:,i] = enc_W[:,i] / np.linalg.norm(enc_W)
...
for i in range(n):
	...
	plt.imshow(feature_images[:,i].reshape(28, 28))
	plt.gray()
